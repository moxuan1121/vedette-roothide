...
- 采样周期可在设置页手动输入 `50–5000ms`，默认 250ms；所有目标仍共用同一个 timer，不为每个 PID 创建线程。
...
应用列表由 Vedette 通过 `LSApplicationWorkspace` 按需读取，并用系统图标接口显示应用图标，不依赖 AltList。赞助、联系、传统监控、CPU throttle 和持续时间设置均已移除。
...
4. 彻底退出并重新打开目标 App。
5. 设置检测周期（默认 250ms），等待 1.5 秒启动宽限期结束，再制造持续 CPU 负载，确认超过阈值后约一个采样周期内退出。
